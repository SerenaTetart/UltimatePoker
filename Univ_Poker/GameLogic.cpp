...
void Game::Bet(int playerNbr,int bet, bool ok) {
	...
	int pos_x = 0, pos_y = 0;
	getPlayerPosition(playerNbr, &pos_x, &pos_y);
	Text* betText = new Text(Properties("", pos_x, pos_y, 1, 1), 26, 0, "+" + std::to_string(bet - betGold), { 255, 215, 0 }, "", {0, 0, 0}, true);
...
void Game::BetPhase(int pos) {
	...
	if (pos == m_Start_pos && m_Player[m_PlayerOrder[pos]]->HasBetted()) GatherPot(); //Si on a fait le tour -> Gather()
		...
		if (pos == 0) {
			//Joueur
			m_PlayerTurn = true;
			std::string tmp = m_GameTurnFrame->GetText();
			m_GameTurnFrame->SetText(28, 0, "Votre tour !", { 255, 0, 0 });
			//Attendre que le joueur est appuyé sur un bouton
			while (m_PlayerTurn == true) {
				SDL_Delay(500);
				if (m_StopThread) return;
			}
			m_GameTurnFrame->SetText(28, 0, tmp, { 255, 215, 0 });
	...
	for (int i = 0; i < m_NbrPlayer; i++) {
		...
		if (!m_Player[m_PlayerOrder[i]]->GetFold() && !m_Player[m_PlayerOrder[i]]->GetSurrender()) {
			...
			if (m_Player[m_PlayerOrder[i]]->GetCardValue(0) >= m_Player[m_PlayerOrder[i]]->GetCardValue(1)) {
				if (score_max < m_Player[m_PlayerOrder[i]]->GetCardValue(0) || (score_max == m_Player[m_PlayerOrder[i]]->GetCardValue(0) && comb_value < m_Player[m_PlayerOrder[i]]->GetCardValue(1)) ) {
					score_max = m_Player[m_PlayerOrder[i]]->GetCardValue(0);
					comb_value = m_Player[m_PlayerOrder[i]]->GetCardValue(1);
					ind_max = i;
				}
			}
			else if (score_max < m_Player[m_PlayerOrder[i]]->GetCardValue(1) || (score_max == m_Player[m_PlayerOrder[i]]->GetCardValue(1) && comb_value < m_Player[m_PlayerOrder[i]]->GetCardValue(0))) {
				score_max = m_Player[m_PlayerOrder[i]]->GetCardValue(1);
				comb_value = m_Player[m_PlayerOrder[i]]->GetCardValue(0);
				ind_max = i;
			}
		}
	}
